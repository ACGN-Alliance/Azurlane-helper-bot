...
from AZbot.plugins.checker.rule_check import bili_handle
from AZbot.plugins.json_utils import JsonUtils as ju
from .bili_article import bili_pic

bili_sub = on_command("bili订阅", rule=bili_handle, permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)
@bili_sub.handle()
async def _(event: GroupMessageEvent, arg: Message=CommandArg()):
    ...
    if len(args) == 0:
        ...
        if args[0] == "开启":
            ...
            path = await bili_pic()
            try:
                await bili_sub.finish(MessageSegment.image(path))
